...
async function runFolderPick() {
  ...
  try {
    const found = await pickAndScanFolder((count) => {
      onbFill.style.width = Math.min(96, 8 + count * 2) + '%';
      onbPct.textContent = String(count);
      onbText.textContent = `Scanning… ${count} found`;
    });
    tracks = found;
...
function updateVuMeters() {
  ...
  const angle = (v) => -32 + Math.min(1, v * 2.2) * 64; // -32deg..+32deg
  needleL.setAttribute('transform', `rotate(${angle(l)} 40 40)`);
  needleR.setAttribute('transform', `rotate(${angle(r)} 40 40)`);
}
...
(async function boot() {
  ...
  try {
    const result = await tryRestoreFolder((count) => {
      onbFill.style.width = Math.min(96, 30 + count * 2) + '%';
      onbText.textContent = `Reopening your library… ${count} found`;
    });
    ...
    if (result.needsPermission) {
      ...
      const onReconnectClick = async () => {
        ...
        onbScan.classList.add('show');
        try {
          tracks = await reconnectFolder((count) => {
            onbFill.style.width = Math.min(96, 8 + count * 2) + '%';
            onbText.textContent = `Reopening your library… ${count} found`;
          });
          onbFill.style.width = '100%';
